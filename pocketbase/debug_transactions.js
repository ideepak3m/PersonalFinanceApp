/**
 * Debug script for transactions import issues
 */
...
async function debugTransactions() {
    ...
    // First, get existing transactions to find which ones succeeded
    const existingRecords = await pb.collection('transactions').getFullList();
    console.log(`📦 Already in PocketBase: ${existingRecords.length}\n`);

    // Try importing all records and log failures in detail
    let successCount = 0;
    let failures = [];
    ...
    if (failures.length > 0) {
        ...
        const errorGroups = {};
        failures.forEach(f => {
            const key = JSON.stringify(f.response);
            if (!errorGroups[key]) errorGroups[key] = [];
            errorGroups[key].push(f);
        });

        for (const [errorKey, items] of Object.entries(errorGroups)) {
            console.log(`\n📋 Error (${items.length} records):`);
            console.log(errorKey);
            console.log('\nSample record:');
            console.log(items[0].record);
            console.log('\nAffected lines:', items.slice(0, 5).map(i => i.line).join(', '), items.length > 5 ? '...' : '');
        }
    }
}
